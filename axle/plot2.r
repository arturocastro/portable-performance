...
data <- read.table(commandArgs(TRUE)[1])
...
{
	datac[n,4] <- min(data[(1 + 10 * (n-1)):(n*10), 3])
	#print(10 * (n-1))
}

datac$V1 <- factor(datac$V1, levels=unique(as.character(datac$V1)) )

opt <- as.integer(commandArgs(TRUE)[3])
...
print(ggplot(datac, aes(x=V2, y=V3, fill=as.factor(V1))) + 
    ...
    ggtitle(paste0("Performance for OpenCL version on ", commandArgs(TRUE)[2])) +
    #ggtitle("what") +
    #scale_y_continuous(breaks=0:20*4, 0.1) +
    theme_bw() + scale_x_discrete(limits = positions)) #+ ylim(0.0, 0.25))
    
 ggsave(filename=paste0(commandArgs(TRUE)[2], ".png"), width=17, limitsize=F)